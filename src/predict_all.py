...
def predict_category(model, word_index, label_dict, max_len, text):
	...
	if (re.search("Karar\sSayısı\s:\sKHK/(\d+)\sKararnamenin", text)) or \
	   (re.search("Karar\sSayısı\s:KHK/(\d+)\sKararnamenin", text)) or \
	   (re.search("Karar\sSayısı:\sKHK/(\d+)\sKararnamenin", text)) or \
	   ("Kanun\sHük.Kar.nin\sTarihi" in text) or \
	   ("Kanun\sHükmünde\sKararnamenin\sTarihi" in text) or \
	   ("Kanun\sHükminde\sKararnamenin\sTarihi" in text) or \
	   ("Kanun\sHük. Kar. nin Tarihi" in text) or \
	   ("Kanun\sHük. Kar.nin Tarihi" in text) or \
	   ("kanun\shük kar nin tarihi" in text) or \
	   ("karar\ssayısı khk kararnamenin" in text) or \
	   ("karar\ssayısı khk kararnamenin" in text) or \
	   ("kanun\shük kar nin tarihi" in text):
		prediction = "Kanun Hükmünde Kararname" 

	elif (re.search("(DAİR\s|İLİŞKİN\s|HAKKINDA\s)(CUMHURBAŞKANLIĞI\sKARARNAMESİ\s\(Kararname\s)(Numarası:\s|Numarası\s:\s|Numarası\s|Numarası\s:|Numarası:)(\d+)\)", text)) or \
			(re.search("(Cumhurbaşkanlığı\sKararnamesinin\s)(Sayısı\s:\s|Sayısı:\s|Sayısı\s:|Sayısı:|Sayısı\s)(\d+)", text)) or \
			(re.search("HAKKINDA CUMHURBAŞKANLIĞI KARARNAMESİ (Kararname Numarası: (\d+))", text)) or \
		...
		if model_prediction == 0:
			...
			if (re.search("BKK\sNo:", text)) or \
				(re.search("bkk\sno", text)) or \
				(re.search("Bakanlar\sKurulu\sKararının\sTarihi\s:\s(\d+)/(\d+)/(\d+),\sNo\s:\s(\d+)/(\d+)", text)) or \
				(re.search("Bakanlar\sKurulu\sKararının\sTarihi\s:\s(\d+)/(\d+)/(\d+)\sNo\s:\s(\d+)/(\d+)", text)) or \
				(re.search("Bakanlar\sKurulu\sKararının\sTarihi\s:\s(\d+).(\d+).(\d+)\sNo\s:\s(\d+)/(\d+)", text)) or \
				(re.search("Bakanlar\sKurulu\sKararının\sTarihi\s:\s(\d+).(\d+).(\d+)\sNo:\s(\d+)/(\d+)", text)) or \
				(re.search("Bakanlar\sKurulu\sKararının\sTarihi\s:\s(\d+).(\d+).(\d+),\sNo\s:\s(\d+)/(\d+)", text)) or \
				(re.search("Bakanlar\sKurulu\sKararının\sTarihi\s:\s(\d+).(\d+).(\d+),\sNo:(\d+)/(\d+)", text)) or \
				(re.search("Dayandığı\sKanunun\sTarihi\s:\s(\d+)/(\d+)/(\d+)", text)) or \
				(re.search("bakanlar\skurulu\skararının\starihi", text)):
				prediction = "Tüzük"
			else:
